...
if __name__ == "__main__":
    N = 50000  # Increased N for even deeper analysis
    ...
    # Analysis for composites (optional, for comparison)
    data_composites = []
    for n in composites:
        info = get_sequence_info(n)
    ...
    plt.ylabel('Density')
    plt.title('Histogram of Sequence Lengths (Primes vs Composites)')
    plt.legend()
    plt.tight_layout()
    plt.savefig("fig_histogram_sequence_lengths.png", dpi=300)
    plt.show()
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig("fig_cycles_graph.png", dpi=300)
    plt.show()

    # ---
    # Export cycle data/results for inclusion in the paper
    # 1. Export cycles as a CSV: each row is a cycle, columns are the cycle elements (padded with empty if needed)
    ...
    df_cycles.to_csv('cycles_multiples_of_9.csv')

    # 2. Export node-to-cycle mapping: each row is (node, cycle_id)
    ...
    df_node_to_cycle.to_csv('node_to_cycle_mapping.csv', index=False)
